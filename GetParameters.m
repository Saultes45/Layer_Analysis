...
% try
    ...
    if ~isempty(MyDirInfo)
        ...
        for cpt_line = 1 : length(Text_from_file) - 1
            ...
            if strcmp(Text_from_file{cpt_line},'[-1]')==0% this is dumb because it should have been avoided w/ the limit of the for loop
                
                if ~strcmp(Text_from_file{cpt_line}(1),'[')
                    %                 read the first part before the "=" sign so we've got the name of the variable
                    [NameOfTheVariable,ValueOfTheVariable] = strtok(Text_from_file{cpt_line}, '='); %#ok<ASGLU>
                    ...
                    if all(ismember(ValueOfTheVariable, '0123456789+-.eEdD')) %then there is a good chance that ValueOfTheVariable is a number
